...
merged2 = txns[txns['status']=='success'].merge(
    users[['user_id','age']], on='user_id')
merged2['age_group'] = pd.cut(
    merged2['age'],
    bins=[12,15,18,21,25],
    labels=['13-15','16-18','19-21','22-25'])
cat_age = merged2.groupby(
    ['age_group','category'],
    observed=True)['txn_id'].count().unstack().fillna(0)

fig5, ax5 = plt.subplots(figsize=(12, 5))
cat_age.plot(kind='bar', ax=ax5, colormap='tab10', edgecolor='white')
ax5.set_title('Transaction Count by Age Group & Category',
